...
export default class UserService {
  ...
  async createUser(username: string, name: string, email: string, password: string): Promise<Omit<User, "password">> {

    const existingUser = await this.userRepository.findByEmail(email)
    if (existingUser) {
      this.logger.debug("Email already in use", { id : existingUser.id , email : existingUser.email})
  ...
  async profileUser (id: string) : Promise<Omit<User, "password">> {
